...
public interface OrderRepository extends JpaRepository<Order,Integer> {
    ...
    @Query(
        value = query + "WHERE quantity < 0 AND market_id = ?1 \n#pageable\n", 
        countQuery = "SELECT COUNT(*) FROM (" + query + "WHERE quantity < 0 AND market_id = ?1" + ") d", 
        nativeQuery = true)
    Page<Order> findSellOrdersByMarket(int marketId, Pageable pageable);

    @Query(
        value = query + "WHERE quantity > 0 AND market_id = ?1 ORDER BY ?#{#pageable}", 
        countQuery = "SELECT COUNT(*) FROM (" + query + "WHERE quantity > 0 AND market_id = ?1", 
        nativeQuery = true)
    ...
    @Query(value = query +
        ...
        countQuery = "SELECT COUNT(*) FROM (" + query + "WHERE quantity < 0 AND market_id = ?2 AND order_value <= ?3 AND portfolio_id != ?1 " + ") d",
        nativeQuery = true)
    ...
    @Query(value = query +
        "WHERE quantity > 0 AND market_id = ?2 AND order_value >= ?3 AND portfolio_id != ?1 " + 
        "ORDER BY order_value",
        countQuery = "SELECT COUNT(*) FROM (" + query + "WHERE quantity > 0 AND market_id = ?2 AND order_value >= ?3 AND portfolio_id != ?1 " + ") d", nativeQuery = true)
